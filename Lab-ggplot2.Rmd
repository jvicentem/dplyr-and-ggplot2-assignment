...
mutate(row, origin_full_name = airport_name$name[1]) %>%
  select(origin, origin_full_name, number_of_flights)
```
...
```{r}
# Primero, vamos a calcular el tiempo de vuelo restando dep_time y arr_time. Sin embargo, dep_time y arr_time tienen time zone local de cada sitio, por lo que es preciso conocer esto antes.
