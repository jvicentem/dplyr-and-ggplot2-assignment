...
time_to_gmt <- function(location, time, year, month, day) {
  tz <- airports %>%
          select(faa, tz) %>%
          filter(faa == location) %>%
          select(tz)

  groups <- str_match_all(time, '(\\d{1,2})(\\d{2})')
  ...
  gmt_time <- as_posixct + as.numeric(tz) * 3600
  
  return(gmt_time)
}

times_diff <- function(time1, time2) {
  return(as.numeric(time1) - as.numeric(time2))
}
```
...
```{r}
# Primero, vamos a obtener el tiempo de vuelo calculando la diferencia de tiempo entre arr_time y dep_time. Sin embargo, dep_time y arr_time tienen time zone local de su respectivo aeropuerto, por lo que es preciso tener en cuenta esto antes.
